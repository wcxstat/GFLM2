...
% m1 = 3;
Y0=Y(U==0); Y1=Y(U==1);
m1=AIC(Y0,xi0,0,[1,14]);
%m1=2;
bh_coef=glmfit(xi0(:,1:m1),Y0,'normal');
...
theta_h1=bisect(Y,U,Int_h,pi_true,0,0,5,0.001); % true
theta_h2=bisect(Y,U,Int_h,pi_lda,0,0,5,0.001); % lda
theta_h3=bisect(Y,U,Int_h,p_h,0,0,5,0.001); % ind
% [theta_h1,~]=newtoneq(Y,U,Int_h,pi_true,0,1,0.001);
% [theta_h2,~]=newtoneq(Y,U,Int_h,pi_lda,0,1,0.001);
% [theta_h3,~]=newtoneq(Y,U,Int_h,p_h,0,1,0.001);

bh_coef1=glmfit(xi1(:,1:m1),Y1,'normal');
% inital value
...
[bh_coef1,~]=coglm(Y,U,xi0,xi1,theta_h1,0,m1,x_in1,0.001);
[bh_coef2,~]=coglm(Y,U,xi0,xi1,theta_h2,0,m1,x_in2,0.001);
[bh_coef3,~]=coglm(Y,U,xi0,xi1,theta_h3,0,m1,x_in3,0.001);
...
Ibd=ibound(Int_h,pi_lda,0,theta_h2);
...
save('asynorm_gauss_alpha2.0.txt','asynorm_mat','-ascii')